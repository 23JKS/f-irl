...
# 第一步：定义 ROERReplayBuffer 类（基于原 ReplayBuffer）
class ROERReplayBuffer(ReplayBuffer):
    def __init__(self, obs_dim, act_dim, device=torch.device('cpu'), size=int(1e6), lambda_=0.01, beta=1.0, clip_min=1.0, clip_max=50.0):
        super().__init__(obs_dim, act_dim, device, size)
        self.priorities = np.ones(size, dtype=np.float32)  # 初始化优先级 d_i = 1
        self.lambda_ = lambda_  # ROER 平滑参数 λ
        self.beta = beta        # KL 温度 β
        self.clip_min = clip_min
        self.clip_max = clip_max

    def update_priorities(self, indices, td_errors):
        """更新优先级 d_i = [λ e^{δ_Q / β} + (1-λ)] d_i"""
        priorities = (self.lambda_ * np.exp(td_errors / self.beta) + (1 - self.lambda_))*self.priorities[indices]
        priorities = np.clip(priorities, self.clip_min, self.clip_max)
        self.priorities[indices] = priorities  # 更新对应索引的优先级
    ...
    def sample_batch(self, batch_size=32):
        ...
        weights = self.priorities[idxs]  # 重要性权重 w_i = d_i (或归一化，根据需要)
        batch = {k: torch.as_tensor(v, dtype=torch.float32).to(self.device) for k,v in batch.items()}
        batch['weights'] = torch.as_tensor(weights, dtype=torch.float32).to(self.device)  # 添加权重
        batch['indices'] = torch.as_tensor(idxs, dtype=torch.long).to(self.device)  # 添加索引，用于更新优先级
        return batch
