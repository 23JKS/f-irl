...
class SMMIRLDisc(nn.Module):
    ...
    def gradient_penalty(self, expert_batch, agent_batch):
        eps = torch.rand((expert_batch.shape[0], 1)).to(self.device)
        ...
        # WGAN-GP style: penalize gradient norm deviating from 1**
    ...
    def learn(self, expert_samples, agent_samples, iter: int):
        ...
        for i in range(iter):
            ...
            if self.grad_pen_weight > 0:
                gp = self.grad_pen_weight * self.gradient_penalty(expert_batch, agent_batch)
                total_loss = total_loss + gp
