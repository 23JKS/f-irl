...
from firl_roer.divs.f_div_disc import f_div_disc_loss
from firl_roer.divs.f_div import maxentirl_loss
from firl_roer.divs.ipm import ipm_loss
from firl_roer.models.reward import MLPReward
from firl_roer.models.discrim import SMMIRLDisc as Disc
...
# Import from sac_roer
from common.sac_roer import ROERReplayBuffer, SAC 
...
if __name__ == "__main__":
    ...
    # logs
    exp_id = f"logs/{env_name}/exp-{num_expert_trajs}/{v['obj']}-roer/{seed}" # task/obj/date structure
    # exp_id = 'debug'
    ...
    os.system(f'cp firl_roer/irl_samples.py {log_folder}')
    ...
    n_itrs = v['irl']['n_itrs']
    roer_start_itr = int(0.8 * n_itrs)  # Last 20% iterations enable ROER

    for itr in range(n_itrs):
        # Trust region: snapshot reward params before this iteration's update
        old_reward_params = {k: v_param.detach().clone()
                            for k, v_param in reward_func.named_parameters()}

        # Enable ROER priority updates only in the last 20% of iterations
        enable_roer_priority = (itr >= roer_start_itr)
        ...
        if v['sac']['reinitialize'] or itr == 0:
            ...
            roer_params = v['sac'].get('roer', {})
            replay_buffer = ROERReplayBuffer(
                state_size,
                action_size,
                device=device,
                size=v['sac']['buffer_size'],
                lambda_=roer_params.get('lambda_', 0.01),
                beta=roer_params.get('beta', 1.0),
                clip_min=roer_params.get('clip_min', 1.0),
                clip_max=roer_params.get('clip_max', 50.0))
            
            if enable_roer_priority:
                print(f"[Iteration {itr}/{n_itrs}] Enabling ROER priority updates (last 20% iterations). "
                      f"Parameters: lambda_={roer_params.get('lambda_', 0.01)}, "
                      f"beta={roer_params.get('beta', 1.0)}, "
                      f"clip_min={roer_params.get('clip_min', 1.0)}, "
                      f"clip_max={roer_params.get('clip_max', 50.0)}")
            else:
                print(f"[Iteration {itr}/{n_itrs}] Using ROER buffer without priority updates "
                      f"(priority updates will be enabled at iteration {roer_start_itr})")
            ...
            sac_agent = SAC(env_fn, replay_buffer,
                ...
                device=device,
                enable_roer_priority=enable_roer_priority,
                **v['sac']
            )
            ...
            # Even if SAC agent is not reinitialized, update the enable_roer_priority flag
            old_enable_roer_priority = sac_agent.enable_roer_priority
            sac_agent.enable_roer_priority = enable_roer_priority
            if enable_roer_priority and not old_enable_roer_priority:
                print(f"[Iteration {itr}/{n_itrs}] Enabling ROER priority updates (last 20% iterations)")
        
        # Only need to change reward in sac
        sac_agent.reward_function = reward_func.get_scalar_reward 
        
        # ==============================================================================
        # [CRITICAL FIX] Force Reset Priorities
        # Since the reward function has changed (or Discriminator updated), the old priorities
        # in the buffer are stale and misleading. We must reset them to 1.0 to allow
        # uniform sampling initially, so the new reward's error can be learned correctly.
        # ==============================================================================
        if hasattr(replay_buffer, 'priorities'):
            # print(f"[Iteration {itr}] Reward updated. Resetting ROER priorities to 1.0.")
            # replay_buffer.priorities.fill(1.0)
            pass
        # ==============================================================================
        ...
        for _ in range(v['reward']['gradient_step']):
            ...
            # Trust region: penalize deviation from previous reward params
            if anchor_lambda > 0:
                anchor_loss = sum(((p - old_reward_params[k]) ** 2).sum()
        ...
        if real_return_det > max_real_return_det and real_return_sto > max_real_return_sto:
            ...
            torch.save(reward_func.state_dict(), os.path.join(logger.get_dir(), 
                    f"model/reward_model_{best_suffix}.pkl"))
            torch.save(sac_agent.ac.state_dict(), os.path.join(logger.get_dir(), 
                    f"model/policy_model_{best_suffix}.pkl"))
