...
def rollout_stats(
    ...
    traj_descriptors: Dict[str, np.ndarray] = {
        "len": np.asarray([len(t.acts) for t in trajectories]),
    }
    if hasattr(trajectories[0], "rews"):
        traj_descriptors["return"] = np.asarray(
            [sum(t.rews) for t in trajectories]  # type: ignore[union-attr]
        )
