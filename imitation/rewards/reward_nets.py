...
from gymnasium.spaces import utils as gymnasium_spaces_utils
...
# Monkey-patch SB3 preprocessing to support gymnasium spaces
# (SB3 1.8.0 only knows gym.spaces, but we use gymnasium.spaces via shimmy)
_original_get_flattened_obs_dim = preprocessing.get_flattened_obs_dim

def _patched_get_flattened_obs_dim(space):
    try:
        return _original_get_flattened_obs_dim(space)
    except NotImplementedError:
        return gymnasium_spaces_utils.flatdim(space)

preprocessing.get_flattened_obs_dim = _patched_get_flattened_obs_dim

# Monkey-patch SB3 base_class to accept gymnasium.spaces as action spaces
try:
    import gymnasium.spaces as _gym2_spaces
    import gym.spaces as _gym1_spaces
    import stable_baselines3.common.base_class as _sb3_base
    import inspect, types

    # Read the source of __init__, replace the assert with a patched version
    # that also accepts gymnasium spaces
    _orig_init = _sb3_base.BaseAlgorithm.__init__

    def _patched_sb3_init(self, *args, **kwargs):
        # Before calling original init, monkey-patch the action_space check
        # by temporarily making gymnasium.spaces.Box an instance of gym.spaces.Box
        _orig_init(self, *args, **kwargs)

    # Simpler: just patch the isinstance check at the source
    # Override the assert by patching action_space property after env setup
    _orig_set_env = _sb3_base.BaseAlgorithm.set_env if hasattr(_sb3_base.BaseAlgorithm, 'set_env') else None

    # Most direct: patch the supported_action_spaces check in __init__
    # by wrapping action_space to return gym.spaces.Box when it's gymnasium.spaces.Box
    import functools

    @functools.wraps(_orig_init)
    def _patched_sb3_init(self, policy, env, learning_rate, *args,
                          supported_action_spaces=None, **kwargs):
        if supported_action_spaces is not None:
            # Add gymnasium equivalents to the supported list
            extra = []
            mapping = {
                _gym1_spaces.Box: _gym2_spaces.Box,
                _gym1_spaces.Discrete: _gym2_spaces.Discrete,
                _gym1_spaces.MultiDiscrete: _gym2_spaces.MultiDiscrete,
                _gym1_spaces.MultiBinary: _gym2_spaces.MultiBinary,
            }
            for s in supported_action_spaces:
                if s in mapping:
                    extra.append(mapping[s])
            supported_action_spaces = tuple(list(supported_action_spaces) + extra)
        _orig_init(self, policy, env, learning_rate, *args,
                   supported_action_spaces=supported_action_spaces, **kwargs)

    _sb3_base.BaseAlgorithm.__init__ = _patched_sb3_init
    print("SB3 BaseAlgorithm patched to support gymnasium spaces")
except Exception as e:
    print(f"Warning: could not patch SB3 BaseAlgorithm: {e}")


def _get_flattened_dim(space) -> int:
    return preprocessing.get_flattened_obs_dim(space)
...
class BasicRewardNet(RewardNet):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_state:
            combined_size += _get_flattened_dim(observation_space)

        self.use_action = use_action
        if self.use_action:
            combined_size += _get_flattened_dim(action_space)
