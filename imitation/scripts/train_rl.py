...
import numpy as np
import torch
# PyTorch 2.6 兼容：默认 weights_only=True，这会破坏 imitation 原有反序列化代码
    ...
    *,
    ...
    with environment.make_venv(  # type: ignore[wrong-keyword-args]
        ...
        def _get_horizon():
            ...
            if "T" in env_make_kwargs:
                return int(env_make_kwargs["T"])
            if getattr(test_env, "spec", None) is not None and test_env.spec.max_episode_steps:
                return int(test_env.spec.max_episode_steps)
            return 1000

        class RealRewardEvalCallback(BaseCallback):
            def __init__(self, eval_env, eval_interval, n_episodes, verbose=0):
                super().__init__(verbose)
                self.eval_env = eval_env
                self.eval_interval = eval_interval
                self.n_episodes = n_episodes
                self.horizon = _get_horizon()

            def _on_step(self) -> bool:
                if self.num_timesteps % self.eval_interval != 0:
                    return True
                real_det = eval.evaluate_real_return(
                    lambda obs, det: _policy_fn(self.model, obs, det),
                    self.eval_env,
                    self.n_episodes,
                    self.horizon,
                    True,
                ...
                print(
                    f"Timestep: {self.num_timesteps} | Real Det: {real_det:.2f} | Real Sto: {real_sto:.2f}"
                )
                return True

        callback_objs.append(RealRewardEvalCallback(test_env, 5000, 10))
