...
def make_vec_env(
    ...
    *,
    ...
    import envs as _custom_envs  # noqa: F401 - registers custom environments (HopperFH-v0, AntFH-v0, etc.)
    # 绕过 gymnasium 的 PassiveEnvChecker（它要求 gymnasium.spaces，但 SB3 要求 gym.spaces）
    import gymnasium.envs.registration as _gym_reg
    import gymnasium.wrappers.env_checker as _gym_checker
    _orig_checker_cls = _gym_reg.PassiveEnvChecker
    _gym_reg.PassiveEnvChecker = lambda env: env
    _gym_checker.PassiveEnvChecker = lambda env: env
    tmp_env = gym.make(env_name)
    _gym_reg.PassiveEnvChecker = _orig_checker_cls
    _gym_checker.PassiveEnvChecker = _orig_checker_cls
    tmp_env.close()
    spec = tmp_env.spec
    env_make_kwargs = env_make_kwargs or {}

    def make_env(i: int, this_seed: int) -> gym.Env:
        import envs as _custom_envs  # noqa: F401 - 子进程中重新注册自定义环境
        import gymnasium.envs.registration as _gym_reg
        import gymnasium.wrappers.env_checker as _gym_checker
        _orig_checker_cls = _gym_reg.PassiveEnvChecker
        _gym_reg.PassiveEnvChecker = lambda env: env
