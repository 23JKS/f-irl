...
if __name__ == "__main__":
    ...
    # 优先走 predict_processed，尽量与 train_adversarial 中训练生成器时的奖励调用一致
    predict_has_update_stats = False
    if hasattr(reward_net, "predict_processed"):
        sig = inspect.signature(reward_net.predict_processed)
        predict_has_update_stats = "update_stats" in sig.parameters

    def reward_fn(obs: np.ndarray, act: np.ndarray,
                  next_obs: np.ndarray, done: np.ndarray) -> np.ndarray:
        if hasattr(reward_net, "predict_processed"):
            if predict_has_update_stats:
                return reward_net.predict_processed(obs, act, next_obs, done, update_stats=False)
