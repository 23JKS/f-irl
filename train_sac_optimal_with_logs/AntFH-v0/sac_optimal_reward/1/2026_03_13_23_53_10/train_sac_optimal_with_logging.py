'''
使用预训练的奖励函数训练SAC（基于train_optimal.py的简洁方法）
通过环境的r参数传入学到的奖励函数，同时记录详细的训练日志
'''
import sys, os, time
import numpy as np
import torch
...
class SACWithLogging(SAC):
    """
    继承SAC类，重写learn_mujoco方法以记录训练过程中的评估结果
    """
    ...
    def learn_mujoco(self, print_out=False, save_path=None):
        ...
        print(f"Training SAC: Total steps {total_steps:d}")
        ...
        for t in range(total_steps):
            ...
            if t % self.log_step_interval == 0:
                ...
                #     if real_det > best_eval:
                #         best_eval = real_det
                #         torch.save(self.ac.state_dict(), save_path)
                #         if print_out:
                #             print(f"  -> Saved best model (Real Det Return: {best_eval:.2f})")
...
if __name__ == "__main__":
    ...
    # ========== 创建环境（关键：使用r参数传入学到的奖励）==========
    train_env = gym.make(env_name, T=env_T, r=reward_func.get_scalar_reward)
    test_env = gym.make(env_name, T=env_T)  # 测试环境使用真实奖励
    
    print("训练环境：使用学到的奖励函数")
    print("测试环境：使用真实环境奖励\n")
    
    # logs
    exp_id = f"train_sac_optimal_with_logs/{env_name}/sac_optimal_reward/{seed}"
    ...
    # 设置环境
    sac_agent.env = train_env  # 训练环境使用学到的奖励
    sac_agent.test_env = test_env  # 测试环境使用真实奖励
    sac_agent.test_fn = sac_agent.test_agent_ori_env  # 使用真实环境评估
    ...
    training_log = sac_agent.learn_mujoco(print_out=True, save_path=model_save_path)
