        ...
        for t in range(total_steps):
            ...
            if should_update:
                for j in range(self.update_every):
                    ...
                    # 延迟启用：前 100k 步用均匀权重让 Q 网络先稳定，之后再开 ROER
                    roer_active = self.enable_roer_priority 

                    if not roer_active:
                        batch['weights'] = torch.ones_like(batch['rew'])

                    # 对齐 JAX: 先 update_critic（SAC更新），再 update_v（用新的 target_critic）
                    self.update(data=batch)

                    if roer_active:
                        # update_v: 使用最新的 target_critic（SAC更新后已经polyak更新过）
                    ...
                    # 步骤 6: 更新优先级
                    if roer_active and indices is not None:
...
if __name__ == "__main__":
    ...
    reward_clamp = v['reward'].get('clamp_magnitude', 10.0)
    def clamped_reward(obs):
        r = reward_func.get_scalar_reward(obs)
        return torch.clamp(r, min=-reward_clamp, max=reward_clamp)

    train_env = gym.make(env_name, T=env_T, r=clamped_reward)
