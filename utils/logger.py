...
class CSVOutputFormat(KVWriter):
    ...
    def writekvs(self, kvs):
        # 第一次写入时，确定列顺序
        if not self.keys:
            if all(k in kvs for k in self._fixed_header):
                # 如果这些字段都在，就按固定顺序
                self.keys = list(self._fixed_header)
                # 如果还有其它字段，再按出现顺序追加在后面
                for k in kvs.keys():
                    if k not in self.keys:
                        self.keys.append(k)
            else:
                # 否则退回到默认：按当前 kvs 的插入顺序
